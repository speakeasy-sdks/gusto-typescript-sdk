  ...
  **/
  getV1CompanyIndustry(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.GetV1CompanyIndustryResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  **/
  putV1CompanyIndustry(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.PutV1CompanyIndustryResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
              res.industry = plainToInstance(
